...
function getUmdBanner(opts: { libraryName: string; external?: string[] }) {
  ...
  let deps: { key: string; val: string }[] = [];
  ...
  const result = createWrapperWithLib({
    depsKeys,
    // depsValKey,
    amdLoader: "define",
    defineDeps,
...
export const createUmdWrapper = (opts: {
  ...
  let pluginExternalDependencies: string[] = [];
